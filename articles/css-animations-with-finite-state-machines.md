...
随着用户界面中可能出现的不同状态和过渡状态数目的在不断增长，样式和动画的管理很快就变得复杂起来。即使是一个简单的登录表单也可以有很多不同的“用户状态流”，并且有许多边界情况需要考虑、
...
让我们从动画的角度来探索一下状态机。假设你在编写一个 loading 动画，它同时只会存在以下四者中任意一个给定的状态。
...
每个箭头告诉我们一个状态的过渡是如何通过事件过渡到另一个状态的，并且有些状态是不可能互相转换的。（比如说你不可能从 success 状态到 failure 状态）。每一个箭头代表一个可以落地的动画，或者可以说是一个过渡状态。
...
这样可以正常工作，但是你必须确保 `is-loading` 类名被移除并且 `is-loaded` 类名被添加，但是更有可能出现的情况是类名变成 `.button.is-loading.is-loaded`。这样可能会导致不符合预期的副作用。
...
- 一个有限状态机同时只能在一种状态中
- 所有的过渡方式必须是确定的，意味着任意给定的状态和时间，必须给出相同的可预测的下一个状态。没有意外。
...
## 声明可视化的状态
目前为止，一切都好。但是现在我们想要防止函数调用在改变状态的时候直接侵入业务逻辑，我们可以创建一个状态机状态转换的函数，包含当前状态和触发事件后转换到的下个状态和返回此状态的逻辑。通过使用 switch 代码块，可能像下面这样：
...
Switch 代码块整理了状态基于事件的转换，我们可以使用对象来简化它：
